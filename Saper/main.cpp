...
int main()
{
    GameFont.openFromFile("assets/Context_Reprise_SSi.ttf");
...
void Game()
{
    ...
    Icon.Decorate = true;
    Icon.DrawCells();
    Icon.Sprite.setPosition({static_cast<float>(WindowWidth) / 2.0f - 25.0f, static_cast<float>(WindowHeight) + 10.0f});

    // Game window creating
    sf::RenderWindow Window(sf::VideoMode({static_cast<unsigned int>(WindowWidth), static_cast<unsigned int>(WindowHeight + 50)}), "Sapper");
    ...
    while (Window.isOpen() && !sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Escape))
    {
        ...
        while (const auto Event = Window.pollEvent())
        {
            ...
            {
                if (MouseReleased->button == sf::Mouse::Button::Right) RightPressed = true;
                if (MouseReleased->button == sf::Mouse::Button::Left) LeftPressed = true;
            }
...
void GridDraw(sf::RenderWindow& Window)
{
    for (int i = WindowWidth / Columns; i < WindowWidth; i += WindowWidth / Columns)
    {
        const sf::Vertex Line[] =
        {
            ...
            {{static_cast<float>(i), static_cast<float>(WindowHeight)}}
        };
    ...
    for (int i = WindowHeight / Rows; i < WindowHeight; i += WindowHeight / Rows)
    {
        const sf::Vertex Line[] =
        {
            ...
            {{static_cast<float>(WindowWidth), static_cast<float>(i)}}
        };
        Window.draw(Line, 2, sf::PrimitiveType::Lines);
    }
}
    ...
    for (int Ym = 0; Ym < Rows; Ym++)
        for (int Xm = 0; Xm < Columns; Xm++)
        {
            ...
            if (RightPressed)
            {
                ...
                if ((*It)->Sprite.getGlobalBounds().contains(
                    {static_cast<float>(PixelPos.x), static_cast<float>(PixelPos.y)}))
                {
                    // Set flag
